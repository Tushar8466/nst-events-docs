...
* **D**enial of Service: Mitigated by rate limiting and lock-free atomic capacity updates (`UPDATE ... WHERE registration_count < max_capacity RETURNING registration_count`) preventing DB deadlocks. (Risk: Medium)
...
**Mitigation — Layer 3: Audit dashboard visibility**
  ...
  * Club Admin can review flagged records and cross-reference with the dispute system. Manual override itself is restricted to Platform Admin via the `manual_mark_attendance` gated RPC (per ADR-005).
